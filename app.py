...
with gr.Blocks(title="Medical RAG Assistant") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("➕ Add Patient"):
            ...
            def add_patient_data(pid, name, age_val, gen, bp_val, hr_val, bs_val, 
                                wt, ht, bmi_val, temp_val, o2_val):
                try:
                    if pm.add_patient(pid, name, int(age_val), gen):
                        measurements = {}
                        if bp_val: measurements["Blood Pressure"] = bp_val
                        if hr_val: measurements["Heart Rate"] = hr_val
                        if bs_val: measurements["Blood Sugar"] = bs_val
                        if wt: measurements["Weight"] = wt
                        if ht: measurements["Height"] = ht
                        if bmi_val: measurements["BMI"] = bmi_val
                        if temp_val: measurements["Temperature"] = temp_val
                        if o2_val: measurements["Oxygen Saturation"] = o2_val
